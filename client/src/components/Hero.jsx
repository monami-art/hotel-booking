...
import { assets, cities } from '../assets/assets'; // Import the cities array as well
...
const Hero = () => {
    ...
    const onSearch = async (e) => {
        ...
        await axios.post('/api/user/store-recent-search', {recentSearchedCity:destination}, {headers: {Authorization: `Bearer ${await
            getToken()}`}});
  ...
  return (
    ...
    bg-[url("/src/assets/heroImage.png")] bg-no-repeat bg-cover br-center h-screen'>
        <p className='bg-[#49B9FF]/50 px-3.5 py-1 rounded-full mt-20'>The UltimateHotel Experience</p>
        ...
        <form onSubmit={onSearch} className='bg-white text-gray-500 rounded-lg px-6 py-4  flex flex-col md:flex-row max-md:items-start gap-4 mt-8 max-md:mx-auto'>
